...
const generateVendorReport = async () => {
  const [data] = await pool.query(`SELECT vendor_name, COUNT(uid) as total_assets, SUM(CASE WHEN condition_lot = 'Good' THEN 1 ELSE 0 END) as good_condition, SUM(CASE WHEN condition_lot = 'Needs Repair' THEN 1 ELSE 0 END) as needs_repair FROM rail_assets WHERE vendor_name IS NOT NULL AND vendor_name != '' GROUP BY vendor_name ORDER BY total_assets DESC;`);
  return data;
};

const generateInventoryReport = async () => {
  const [data] = await pool.query(`SELECT asset_type, COUNT(uid) as total_units, AVG(DATEDIFF(warranty_expiry, CURDATE())) as avg_days_left_warranty FROM rail_assets WHERE asset_type IS NOT NULL AND asset_type != '' GROUP BY asset_type ORDER BY total_units DESC;`);
  return data;
};

const getVendorContext = async (vendorName) => {
  if (!vendorName) return null;
  const [data] = await pool.query(`SELECT COUNT(uid) as total_assets, SUM(CASE WHEN condition_lot = 'Good' THEN 1 ELSE 0 END) as good_condition, SUM(CASE WHEN condition_lot = 'Needs Repair' THEN 1 ELSE 0 END) as needs_repair FROM rail_assets WHERE vendor_name = ?;`, [vendorName]);
  return data[0];
};

const getAssetTypeContext = async (assetType) => {
  if (!assetType) return null;
  const [data] = await pool.query(`SELECT COUNT(uid) as total_units, AVG(DATEDIFF(warranty_expiry, CURDATE())) as avg_days_left_warranty FROM rail_assets WHERE asset_type = ?;`, [assetType]);
  return data[0];
};
...
const getVendorLotStatusSummary = async (vendorName) => {
  if (!vendorName) return null;
  const [items] = await pool.query('SELECT * FROM rail_assets WHERE vendor_name = ?', [vendorName]);
